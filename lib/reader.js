...
export function makeReader({ base, fid, fetchImpl, neynarKey = '' }) {
  ...
  return {
    ...
    // Neynar v2 documented path is /feed/mentions; older builds expose
    // /notifications. Try the documented one first, fall back, unwrap either shape.
    async mentions(limit = 25) {
      if (!fid) return [];
      let d = await get(`/v2/farcaster/feed/mentions?fid=${fid}&limit=${limit}`);
      if (!d) d = await get(`/v2/farcaster/notifications?fid=${fid}&type=mentions&limit=${limit}`);
      const items = d?.casts ?? d?.notifications ?? [];
...
export function normalizeCast(c = {}) {
  return {
    ...
    reactions: {
      // Only true like counts (#9) - reactions.count is a total, not likes.
      likes: c.reactions?.likes_count ?? c.likes ?? 0,
