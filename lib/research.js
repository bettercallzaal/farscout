...
// special-casing here.
async function gatherSignal({ reader, channels, watchFids, reddit = null, subreddits = [], watchRedditors = [], x = null, xHandles = [] }) {
  const [casts, channelCasts, trending, mentions, watched, subredditPosts, redditorPosts, xPosts] = await Promise.all([
    ...
    x && xHandles.length ? safe(x.timeline?.(xHandles, 10)) : [],
  ]);
  const all = [...casts, ...channelCasts, ...trending, ...mentions, ...watched, ...subredditPosts, ...redditorPosts, ...xPosts];
...
async function gatherSources({ search, topic, seedUrls = [], queries = null }) {
  ...
  const hitGroups = await Promise.all(
    qs.flatMap((q) => [
      ...
      // X posts as citable sources (no-op/[] without a Nitter instance).
      search.searchX ? safe(search.searchX(q, Math.max(3, perQ))) : [],
