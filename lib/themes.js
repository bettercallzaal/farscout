...
//   xHandles       - X / Twitter handles to read (needs a Nitter instance)
//   standingTopics - topics always researched each cycle, regardless of signal
//
// Explicit WATCH_CHANNELS / WATCH_SUBREDDITS / WATCH_X_HANDLES / STANDING_TOPICS
// env values are MERGED on top of the resolved themes (config.js), never replaced
// - so a theme is a starting point you can always extend.
export const THEME_PRESETS = {
  farcaster: {
    channels: ['zao', 'dev', 'miniapps'],
    subreddits: ['farcaster'],
    xHandles: ['farcaster_xyz', 'dwr'],
  ...
  gamestop: {
    ...
    xHandles: ['GameStop', 'gstopcorp'],
    standingTopics: ['gamestop-stock', 'gamestop-crypto-wallet', 'gamestop-nft-marketplace'],
  },
  // ZAO growth: converges the Pi researchers (YouTube growth, SEO/web-presence,
  // repo/site improvement) onto Farscout's grounded engine. THEMES=zao-growth.
  'zao-growth': {
    channels: ['zao', 'music', 'wavewarz'],
    subreddits: ['NewTubers', 'SEO', 'youtubers'],
    xHandles: ['bettercallzaal', 'WaveWarZ'],
    standingTopics: ['youtube-growth-music-web3', 'seo-web-presence-zao', 'wavewarz-shorts-clips', 'zao-site-improvements'],
  },
};

export const KNOWN_THEMES = Object.keys(THEME_PRESETS);

const FIELDS = ['channels', 'subreddits', 'xHandles', 'standingTopics'];
...
export function resolveThemes(names = []) {
  const out = { channels: [], subreddits: [], xHandles: [], standingTopics: [], unknown: [] };
